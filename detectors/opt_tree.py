    ...
    def index_merged_node(self, hirenodes, num_of_bin):
        ...
        if num_of_bin == 2:
            ...
            if len(hirenodes)<3:
                return
            n = len(hirenodes)
            centroid = np.random.choice(n, 1)[0]
            points = sorted(range(n), key=lambda i: self.get_weight_distance(hirenodes, [i, centroid, centroid]))[1:3]
            ...
            while True:
                ...
                for i in range(n):
                    for j in range(3):
                        temp = points.copy()
                        temp[j] = i
                        if len(set(temp)) == 3 and self.get_weight_distance(hirenodes, temp) < self.get_weight_distance(hirenodes, new_points):
                            new_points = temp
